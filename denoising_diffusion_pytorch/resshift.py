...
class Unet(Module):
    ...
    def forward(self, x, time, x_self_cond = None):
        # TODO need to add low_res image condition
...
def exponential_beta_schedule(num_diffusion_timesteps, min_noise_level, etas_end: float =0.99, kappa : float =1.0, power: float =1.0):
    ...
    increaser = math.exp(1 / (num_diffusion_timesteps - 1) * math.log(etas_end / betas_start))
...
class GaussianDiffusion(Module):
    def __init__(
        ...
        model,
        *,
        sr_factor: int = 4, # super resolution factor
        timesteps = 15,
        sampling_timesteps = None,
        objective = 'pred_x0', # default in ResShift
        beta_schedule = 'exponential',
        schedule_fn_kwargs = dict(), # specify the kappa parameter for exponential beta schedule
        auto_normalize = True,
        offset_noise_strength = 0.,  # https://www.crosslabs.org/blog/diffusion-with-offset-noise
        min_snr_loss_weight = False, # https://arxiv.org/abs/2303.09556
        min_snr_gamma = 5,
        latent_model : nn.Module | None = None, # Optional Latent Diffusion Model to run ResShift in latent space
        ...
        # latent model should remain frozen
        ...
        assert sr_factor > 1, 'super resolution factor should be greater than 1'
        ...
        if beta_schedule == 'linear':
            ...
            beta_schedule_fn = exponential_beta_schedule
        else:
            raise ValueError(f'unknown beta schedule {beta_schedule}')

        self.kappa = schedule_fn_kwargs.get('kappa', 1.0)
        ...
        register_buffer('sqrt_betas', sqrt_betas)
        ...
        if objective == 'pred_noise':
            ...
            # there are no weights used for pred_x0
    ...
    def model_predictions(self, x, lr_img, t, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False):
        """Model predictions for a given time step.
        
        Args:
            x: Tensor, the noisy input image tensor.
            t: int, the time step.
            lr_img: Tensor, the low resolution image tensor.
        """
        print("Model input \n", x.min(), x.max(), x.mean(), x.std())
        model_output = self.model(self._scale_input(x, t), t, lr_img)


        print("Model output \n", model_output.min(), model_output.max(), model_output.mean(), model_output.std())
    ...
    def p_mean_variance(self, x, lr_img: Tensor, t, x_self_cond = None, clip_denoised = True):
        preds = self.model_predictions(x, lr_img, t, x_self_cond)
    ...
    def encode_img(self, latent_model, img: Tensor, up_sample=False) -> Tensor:
        """Upsample and encode to latent or just up sample low res image."""
        # https://github.com/zsyOAOA/ResShift/blob/dfc2ff705a962de1601a491511b43a93b97d9622/models/gaussian_diffusion.py#L500
        if up_sample:
            img = F.interpolate(img, scale_factor=self.sr_factor, mode='bicubic')
        if latent_model:
            with torch.no_grad():
    ...
    def decode_latent(self, z: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            # avoid memory issues
            return torch.cat([self.latent_model.decode(z_.unsqueeze(0) / self.latent_scale_factor) for z_ in z], dim=0)
    ...
    def prior_sample(self, y, noise=None):
        ...
        return y + extract(self.kappa * self.sqrt_betas, t, y.shape) * noise

    def _scale_input(self, inputs, t):
        """Scale inputs after q_sample."""
        # https://github.com/zsyOAOA/ResShift/blob/dfc2ff705a962de1601a491511b43a93b97d9622/models/gaussian_diffusion.py#L598
        if self.scale_input:
            if self.latent_model:
                std = torch.sqrt(extract(self.betas, t, inputs.shape) * self.kappa**2 + 1)
                inputs_norm = inputs / std
            else:
                inputs_max = extract(self.betas**0.5, t, inputs.shape) * self.kappa * 3 + 1
    ...
    def p_sample(self, x, lr_img: Tensor, t: int, x_self_cond = None):
        b, *_, device = *x.shape, self.device
        batched_times = torch.full((b,), t, device = device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, lr_img=lr_img, t = batched_times, x_self_cond = x_self_cond, clip_denoised = (self.latent_model is None))
    ...
    def p_sample_loop(self, lr_img: Tensor, return_all_timesteps = False):
        ...
        # lr_img = self.encode_img(self.latent_model, lr_img, up_sample = True)
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, lr_img, t, self_cond)
            imgs.append(img)
    ...
    @torch.inference_mode()
    def run_super_res(self, lr_img: Tensor, return_all_timesteps = False):
        self.setup_seed()
        self.model.eval()
        return self.p_sample_loop(lr_img, return_all_timesteps = return_all_timesteps)
    ...
    def q_sample(self, x_start, lr_img, t, noise = None):
        ...
        return (
            extract(self.sqrt_alphas_cumprod, t, x_start.shape) * (lr_img - x_start) + x_start +
    ...
    def forward(self, hr_img, lr_img, *args, **kwargs):
        """Compute loss of diffusion process.
        
        Args:
            hr_img: High resolution image tensor.
            lr_img: Low resolution image tensor.
        """
        b, c, h, w, device = *hr_img.shape, hr_img.device
    
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()

        # img = self.normalize(img)
        return self.p_losses(hr_img, lr_img, t, *args, **kwargs)
...
def save_super_resolution_preds(lr_imgs, hr_imgs, filename):
    """Save low-resolution and high-resolution images side by side."""
    # Convert tensors to numpy arrays
    lr_imgs = lr_imgs.cpu().numpy().transpose(0, 2, 3, 1)
    hr_imgs = hr_imgs.cpu().numpy().transpose(0, 2, 3, 1)

    # images are in range -1 to 1, convert to 0 to 1 to avoid warning
    lr_imgs = ((lr_imgs + 1) / 2).clip(0, 1)
    hr_imgs = ((hr_imgs + 1) / 2).clip(0, 1)
    
    # Create a figure
    fig, axes = plt.subplots(nrows=lr_imgs.shape[0], ncols=2, figsize=(10, 5 * lr_imgs.shape[0]))
...
class Trainer:
    def __init__(
        ...
        *,
        ...
        # dataset and dataloader for super-resolution
        ...
        # check that latent model is not included in the to be optimized parameters
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    batch = self.dm.process_data(next(self.dl), phase="train")
                    lr_img , hr_img = batch["lq"], batch["gt"]
                    lr_img, hr_img = lr_img.to(device), hr_img.to(device)

                    with self.accelerator.autocast():
                        loss = self.model(hr_img=hr_img, lr_img=lr_img)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        with torch.inference_mode():
                            ...
                            batch = self.dm.process_data(next(iter(self.val_dl)), phase="val")
                            lr_img = batch["lq"].to(device)
                            hr_imgs = self.ema.ema_model.run_super_res(lr_img, return_all_timesteps = False)
                            save_super_resolution_preds(lr_img, hr_imgs, self.results_folder / f'sr-{milestone}.png')
